...
import (
	"bufio"
	"fmt"
	"github.com/mpppk/atcoder/done/abc158/D"
	"io"
	"os"
	"strings"
)
...
func solve(input *D.lib_Input) string {
	...
	s := D.lib_ReverseStr(strings.Join(front, "")) + S + strings.Join(back, "")
	if reverse {
		return D.lib_ReverseStr(s)
	} else {
		return s
	}
}
