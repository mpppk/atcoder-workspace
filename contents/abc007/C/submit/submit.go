...
func (i *Input) GetLine(index int) ([]string, // GetLine は指定された行を返します. 存在しない行がある場合、失敗します.
	error) {
	if err := i.validateRowIndex(index); err != nil {
		return nil, err
	}
...
func (i *Input) MustReadAsStringGridFrom(fromIndex int) [][]string { // MustReadAsStringGridFrom は指定された行以降の行を、一文字ずつのsliceとして返します.
...
func //
lib_NewIntGridMap(row, col int, defaultValue int) (m [][]int) {
	for i := 0; i < row; i++ {
		var newLine []int
		for j := 0; j < col; j++ {
