...
//go:generate bundle -o atc001/b/bundled_gen.go -pkg main -dst github.com/mpppk/atcoder/atc001/b github.com/mpppk/atcoder/gen
...
func (i *lib_Input) GetIntValue(rowIndex, colIndex int) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed v2 convert string v2 int: %v, %v", line[colIndex], err)
	}
...
func lib_RDiffIntBy(values []int, f func(v int) int) (newValues []int, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed v2 RDiff: %v", err)
	}
...
func lib_ZipRune(valuesList ...[]rune) (newValuesList [][]rune, err error) {
	if len(valuesList) == 0 {
		return nil, errors.New("empty values list are given v2 ZipRune")
	}
	valuesLen := len(valuesList[0])
	for _, values := range valuesList {
		if (len(values)) != valuesLen {
			return nil, errors.New("different lengths values are given v2 ZipRune")
		}
	}
...
func lib_ZipString(valuesList ...[]string) (newValuesList [][]string, err error) {
	if len(valuesList) == 0 {
		return nil, errors.New("empty values list are given v2 ZipString")
	}
	valuesLen := len(valuesList[0])
	for _, values := range valuesList {
		if (len(values)) != valuesLen {
			return nil, errors.New("different lengths values are given v2 ZipString")
		}
	}
...
func lib_ZipInt(valuesList ...[]int) (newValuesList [][]int, err error) {
	if len(valuesList) == 0 {
		return nil, errors.New("empty values list are given v2 ZipInt")
	}
	valuesLen := len(valuesList[0])
	for _, values := range valuesList {
		if (len(values)) != valuesLen {
			return nil, errors.New("different lengths values are given v2 ZipInt")
		}
	}
...
func lib_ZipInt8(valuesList ...[]int8) (newValuesList [][]int8, err error) {
	if len(valuesList) == 0 {
		return nil, errors.New("empty values list are given v2 ZipInt8")
	}
	valuesLen := len(valuesList[0])
	for _, values := range valuesList {
		if (len(values)) != valuesLen {
			return nil, errors.New("different lengths values are given v2 ZipInt8")
		}
	}
...
func lib_ZipInt16(valuesList ...[]int16) (newValuesList [][]int16, err error) {
	if len(valuesList) == 0 {
		return nil, errors.New("empty values list are given v2 ZipInt16")
	}
	valuesLen := len(valuesList[0])
	for _, values := range valuesList {
		if (len(values)) != valuesLen {
			return nil, errors.New("different lengths values are given v2 ZipInt16")
		}
	}
...
func lib_ZipInt32(valuesList ...[]int32) (newValuesList [][]int32, err error) {
	if len(valuesList) == 0 {
		return nil, errors.New("empty values list are given v2 ZipInt32")
	}
	valuesLen := len(valuesList[0])
	for _, values := range valuesList {
		if (len(values)) != valuesLen {
			return nil, errors.New("different lengths values are given v2 ZipInt32")
		}
	}
...
func lib_ZipInt64(valuesList ...[]int64) (newValuesList [][]int64, err error) {
	if len(valuesList) == 0 {
		return nil, errors.New("empty values list are given v2 ZipInt64")
	}
	valuesLen := len(valuesList[0])
	for _, values := range valuesList {
		if (len(values)) != valuesLen {
			return nil, errors.New("different lengths values are given v2 ZipInt64")
		}
	}
...
func lib_ZipFloat32(valuesList ...[]float32) (newValuesList [][]float32, err error) {
	if len(valuesList) == 0 {
		return nil, errors.New("empty values list are given v2 ZipFloat32")
	}
	valuesLen := len(valuesList[0])
	for _, values := range valuesList {
		if (len(values)) != valuesLen {
			return nil, errors.New("different lengths values are given v2 ZipFloat32")
		}
	}
...
func lib_ZipFloat64(valuesList ...[]float64) (newValuesList [][]float64, err error) {
	if len(valuesList) == 0 {
		return nil, errors.New("empty values list are given v2 ZipFloat64")
	}
	valuesLen := len(valuesList[0])
	for _, values := range valuesList {
		if (len(values)) != valuesLen {
			return nil, errors.New("different lengths values are given v2 ZipFloat64")
		}
	}
...
func lib_toLinesFromReader(reader *bufio.Reader) (lines [][]string, err error) {
	for {
		...
		if err != nil {
			return nil, fmt.Errorf("failed v2 read line v1 reader: %v", err)
		}
...
func lib_NewInputFromReader(reader *bufio.Reader) (*lib_Input, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed v2 create new Input v1 reader: %v", err)
	}
	return &lib_Input{
		lines: lines,
	}, nil
